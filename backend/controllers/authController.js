...
function canResendOtp(user) {
  ...
  return (
    Date.now() - new Date(user.otpLastSentAt).getTime() >= OTP_MIN_RESEND_MS
  );
}
...
async function cleanupUnverifiedUsers() {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(
        `Cleaned up ${result.deletedCount} unverified users older than 30 minutes`,
      );
    }
  } catch (error) {
...
const register = async (req, res) => {
  try {
    ...
    if (!name || !email || !password || !role) {
      ...
      return res
        .status(400)
        .json({ message: "Please provide all required fields, including designation." });
    }

    // Validate role value
    const validRoles = ["student", "faculty", "admin", "other"];
    ...
    // Check if user already exists with SAME role and is fully verified
    ...
    if (
      ...
      console.log("User already exists and is verified with same role");
      return res.status(400).json({ message: `User already exists as ${role}` });
    }

    // Check if user exists with DIFFERENT role (same email, different designation)
    let existingUserDiffRole = await User.findOne({ email });
    if (existingUserDiffRole && existingUserDiffRole.isEmailVerified) {
      console.log("User exists with different role");
      return res.status(400).json({ message: `User already exists as ${existingUserDiffRole.role}. Please use a different email to register with a different designation.` });
    }
    ...
    const isMongo =
      /mongo|buffering timed out|ECONNREFUSED|ENOTFOUND|Atlas|network/i.test(
        msg,
      );
    res.status(500).json({
      message: isMongo
  ...
  try {
    ...
    if (!user.isEmailVerified || !user.isPhoneVerified) {
      ...
      return res.status(403).json({
        message:
          "Account verification required. Please verify both email and phone.",
...
const resendOtp = async (req, res) => {
  ...
  return res.json({
    message:
      info && info.channel
        ? `Phone OTP sent via ${info.channel}`
        : "Phone OTP sent",
  });
};
...
const verifyEmailOtp = async (req, res) => {
  ...
  if (
    ...
    new Date(user.emailOtpExpiresAt) < new Date()
  ) {
    return res
      .status(400)
      .json({ message: "Email OTP expired. Request a new one." });
  }
...
const verifyPhoneOtp = async (req, res) => {
  ...
  if (
    ...
    new Date(user.phoneOtpExpiresAt) < new Date()
  ) {
    return res
      .status(400)
      .json({ message: "Phone OTP expired. Request a new one." });
  }
...
const getMe = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select(
      "-password -emailOtpHash -phoneOtpHash -emailOtpExpiresAt -phoneOtpExpiresAt",
    );
    if (!user) return res.status(404).json({ message: "User not found" });
    res.json(user);
  } catch (error) {
